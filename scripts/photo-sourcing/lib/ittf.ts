// ITTF results profile scraper (TT-200).
//
// One GET per player. The page is a public results.ittf.link profile and
// returns HTML with a "Style:" string we parse for handedness + grip.
// Used only for new-player enrichment after the WTT roster scan; existing
// players keep whatever they already have in the local DB.
//
...
//
// Unknown-data fallback used by ITTF for incomplete profiles:
...
//
// We map "Unknown ..." segments to null so the scan still records a row
// with whatever we DID get (e.g. birth year present, style absent).

const ITTF_PROFILE_URL =
...
// Pure parser. Exposed so unit tests can drive it with HTML fixtures
...
export function parseIttfProfile(html: string): IttfProfile {
  ...
  if (birthMatch) {
    ...
    // field null than splice garbage into seed.sql.
...
// Pace ITTF requests at ≥1s/req per IP. Same posture as Wikimedia.
...
export async function fetchIttfProfile(
  ...
  const url = `${ITTF_PROFILE_URL}?resetfilters=1&vw_profiles___player_id_raw=${ittfid}`;
