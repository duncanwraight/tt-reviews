...
describe('Discord Integration - Core Functionality', () => {
  describe('Discord Service Core Functions', () => {
    it('should correctly parse and format equipment search results', async () => {
      ...
      const formatEquipmentResults = (
        equipment: typeof mockEquipment,
        query: string,
        siteUrl: string
      ) => {
    ...
    it('should correctly format player search results', async () => {
      ...
      const formatPlayerResults = (players: typeof mockPlayers, query: string, siteUrl: string) => {
        if (players.length === 0) {
          return {
    ...
    it('should handle empty search results correctly', () => {
      const formatEquipmentResults = (equipment: any[], query: string) => {
        if (equipment.length === 0) {
          return {
            content: `🔍 No equipment found for "${query}"`,
          }
    ...
    it('should correctly format Discord webhook notification payloads', () => {
      const createNotificationPayload = (reviewData: any) => {
        const embed = {
  ...
  describe('Discord Permission System', () => {
    it('should correctly validate user roles', () => {
      ...
      expect(
        checkUserPermissions({ roles: ['moderator', 'other-role'] }, ['moderator', 'admin'])
      ).toBe(true)
    ...
    it('should format approval responses correctly', () => {
      ...
      // Test first approval
      const firstApproval = formatApprovalResponse(
        'first_approval',
        'moderator1',
        'review-123',
      ...
      // Test full approval
      const fullApproval = formatApprovalResponse(
        'fully_approved',
        'moderator2',
        'review-123',
        'Review fully approved and published!'
      )
      ...
      const alreadyApproved = formatApprovalResponse(
        'already_approved',
        'moderator1',
        'review-123',
        'You have already approved this review'
      )
      expect(alreadyApproved.content).toContain(
        '⚠️ **moderator1**: You have already approved this review'
      )
  ...
  describe('Two-Review Approval Logic', () => {
    it('should correctly determine approval status transitions', () => {
      ...
      const secondApproval = simulateApprovalFlow(
  ...
  describe('Discord Interaction Types', () => {
    it('should correctly identify Discord interaction types', () => {
      const handleInteractionType = (interaction: any) => {
    ...
    it('should parse button interactions correctly', () => {
      const parseButtonInteraction = (customId: string) => {
