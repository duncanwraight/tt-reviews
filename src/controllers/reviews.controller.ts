...
export class ReviewsController {
    ...
    try {
      ...
      if (!body.equipment_id || !body.overall_rating || !body.category_ratings) {
        return createErrorResponse(
          c,
          'Missing required fields: equipment_id, overall_rating, category_ratings',
          400
        )
      }
      ...
      const review = await this.equipmentService.createReview(
        user.id,
        body.equipment_id,
        body.overall_rating,
        body.category_ratings,
        body.review_text,
        body.reviewer_context
      )
  ...
  async updateReview(c: Context) {
    try {
      ...
      const review = await this.equipmentService.updateReview(
        reviewId,
        user.id,
        body.overall_rating,
        body.category_ratings,
