    ...
    >
      ...
      <section class="py-8">
        <div class="main-container">
          ...
          <div class="add-equipment-section mt-12 pt-8 border-t border-gray-200">
  ...
  return `
    async function handleEquipmentSetupSubmit(event, playerSlug) {
      ...
      if (!equipmentSetup.year) {
        alert('Year is required for equipment setups');
        return;
      }
      ...
      try {
        ...
        if (result.success) {
          alert('Equipment setup added successfully!');
          form.reset();
          // Reset hidden inputs
          document.getElementById('new-blade-id').value = '';
          document.getElementById('new-forehand-rubber-id').value = '';
          document.getElementById('new-backhand-rubber-id').value = '';
        } else {
          alert('Error: ' + result.message);
        }
      } catch (error) {
        console.error('Submit error:', error);
        alert('An error occurred while adding the equipment setup.');
      }
    }
  `
}
    ...
    async function handlePlayerSubmit(event, mode) {
      ...
      try {
        ...
        if (result.success || result.id) {
          ...
          alert(mode === 'update' ? 'Player updated successfully!' : 'Player submitted successfully!');
          if (result.slug || result.data?.slug) {
            window.location.href = '/players/' + (result.slug || result.data.slug);
          } else {
