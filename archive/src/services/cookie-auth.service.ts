...
export class CookieAuthService extends AuthWrapperService {
  ...
  private readonly COOKIE_MAX_AGE = 60 * 60 * 24 * 7 // 7 days
   ...
   */
  async getOptionalAuthContext(c: Context): Promise<AuthContext | null> {
    try {
   ...
   */
  async signOutWithCookie(c: Context): Promise<{ error: Error | null }> {
    try {
