export class EquipmentService {
    ...
    async createReview(userId, equipmentId, overallRating, categoryRatings, reviewText, reviewerContext) {
        const { data, error } = await this.supabase
            .from('equipment_reviews')
            .insert({
            user_id: userId,
    ...
    async getUserReview(userId, equipmentId) {
        const { data, error } = await this.supabase
            .from('equipment_reviews')
            .select(`
        *,
    ...
    async updateReview(reviewId, userId, overallRating, categoryRatings, reviewText, reviewerContext) {
        const updateData = {};
        ...
        if (reviewerContext !== undefined)
            updateData.reviewer_context = reviewerContext;
        const { data, error } = await this.supabase
            .from('equipment_reviews')
            .update(updateData)
            .eq('id', reviewId)
    ...
    async submitEquipment(userId, equipmentData) {
