...
describe('DiscordService', () => {
    ...
    describe('verifySignature', () => {
        it('should verify valid Discord signature', async () => {
            const result = await discordService.verifySignature('mock-signature', '1234567890', '{"type": 1}');
        ...
        it('should reject invalid signature', async () => {
            ...
            const result = await discordService.verifySignature('invalid-signature', '1234567890', '{"type": 1}');
        ...
        it('should throw error when DISCORD_PUBLIC_KEY is not configured', async () => {
            ...
            await expect(serviceWithoutKey.verifySignature('sig', 'timestamp', 'body')).rejects.toThrow('DISCORD_PUBLIC_KEY not configured');
        });
    });
    describe('handleSlashCommand', () => {
        it('should handle equipment search command', async () => {
            ...
            vi.mocked(EquipmentService).mockImplementation(() => mockEquipmentService);
            const interaction = {
                data: {
                    name: 'equipment',
        ...
        it('should handle approve command with full approval', async () => {
            ...
            const moderationService = discordService.moderationService;
    ...
    describe('handlePrefixCommand', () => {
        ...
        it('should handle !player prefix command', async () => {
            ...
            vi.mocked(PlayerService).mockImplementation(() => mockPlayerService);
            const message = {
    ...
    describe('notifyNewReview', () => {
        it('should send Discord webhook notification', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith(mockEnv.DISCORD_WEBHOOK_URL, expect.objectContaining({
                method: 'POST',
        ...
        it('should throw error when DISCORD_WEBHOOK_URL is not configured', async () => {
            ...
            await expect(serviceWithoutWebhook.notifyNewReview({
                id: 'review-123',
    ...
    describe('handleMessageComponent', () => {
        it('should handle approve button click', async () => {
            ...
            vi.mocked(ModerationService).mockImplementation(() => mockModerationService);
            const interaction = {
    ...
    describe('checkUserPermissions', () => {
        it('should allow users with correct roles', async () => {
            ...
            const result = await service.checkUserPermissions({ roles: ['role1', 'other-role'] }, 'guild-123');
            expect(result).toBe(true);
        });
        ...
        it('should allow all users when no roles are configured', async () => {
            ...
            const service = serviceWithoutRoles;
    ...
    describe('notifyNewPlayerEdit', () => {
        it('should send player edit notification with correct format', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith('https://discord.com/api/webhooks/mock', expect.objectContaining({
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        it('should throw error when webhook URL not configured', async () => {
            ...
            await expect(serviceWithoutWebhook.notifyNewPlayerEdit(editData)).rejects.toThrow('DISCORD_WEBHOOK_URL not configured');
        });
    });
});
