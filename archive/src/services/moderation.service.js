export class ModerationService {
    ...
    async approveReview(reviewId, moderatorId, isAdminApproval = false) {
        try {
    ...
    async getModerationStats() {
        const [pendingResult, approvedResult, rejectedResult, playerPendingResult, playerApprovedResult, playerRejectedResult, equipmentPendingResult, equipmentApprovedResult, equipmentRejectedResult,] = await Promise.all([
            this.supabase
                .from('equipment_reviews')
                .select('*', { count: 'exact', head: true })
                .eq('status', 'pending'),
        ...
        return {
            ...
            equipmentSubmissionsTotal: equipmentSubmissionsPending + equipmentSubmissionsApproved + equipmentSubmissionsRejected,
        };
    }
    ...
    async approvePlayerEdit(editId, moderatorId) {
        try {
    ...
    async rejectPlayerEdit(editId, moderatorId, reason) {
        const { error } = await this.supabase
            .from('player_edits')
            .update({
            status: 'rejected',
    ...
    async getPlayerEditById(editId) {
    ...
    async getPendingEquipmentSubmissions(limit = 50, offset = 0) {
        ...
        const total = countResult.count || 0;
        const equipmentSubmissions = submissionsResult.data || [];
        return { equipmentSubmissions, total };
    }
    async approveEquipmentSubmission(submissionId, moderatorId) {
        try {
            ...
            await this.logModerationAction(submissionId, moderatorId, 'approved', undefined, 'equipment_submission');
            return {
                success: true,
                status: 'approved',
    ...
    async rejectEquipmentSubmission(submissionId, moderatorId, reason) {
        const { error } = await this.supabase
            .from('equipment_submissions')
            .update({
            status: 'rejected',
        ...
        await this.logModerationAction(submissionId, moderatorId, 'rejected', reason, 'equipment_submission');
        return true;
    }
    async getEquipmentSubmissionById(submissionId) {
        const { data, error } = await this.supabase
            .from('equipment_submissions')
            .select('*')
            .eq('id', submissionId)
    ...
    async getReviewApprovals(_reviewId) {
    ...
    async logModerationAction(itemId, moderatorId, action, reason, itemType = 'review') {
        const logEntry = {
            itemId,
            moderatorId,
            action,
        ...
        console.log(`Moderation action: ${action} ${itemType} ${itemId} by moderator ${moderatorId}`, logEntry);
