 ...
 */
export async function secureAuth(c: Context<{ Variables: SecureAuthVariables }>, next: Next) {
 ...
 */
export async function optionalSecureAuth(
  c: Context<{ Variables: Partial<SecureAuthVariables> }>,
  next: Next
) {
  const env = validateEnvironment(c.env)
  const authService = new CookieAuthService(env)
...
export async function requireSecureAdmin(
  c: Context<{ Variables: SecureAuthVariables }>,
  next: Next
) {
  const env = validateEnvironment(c.env)
  const authService = new CookieAuthService(env)
 ...
 */
export async function requireCSRF(c: Context<{ Variables: SecureAuthVariables }>, next: Next) {
  ...
  const csrfToken =
    ...
    (await c.req
      .formData()
      .then(fd => fd.get('csrf_token') as string)
      .catch(() => undefined))
 ...
 */
export async function injectCSRF(c: Context<{ Variables: SecureAuthVariables }>, next: Next) {
  const authService = c.get('authService')
  if (authService) {
