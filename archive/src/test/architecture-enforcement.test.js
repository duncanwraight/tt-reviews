...
describe('Architecture Enforcement', () => {
    describe('Database Access Patterns', () => {
        it('should enforce InternalApiService usage in routes', () => {
            ...
            for (const file of [...routeFiles, appFile]) {
                ...
                const hasDirectSupabaseCreation = content.includes('createClient(env.SUPABASE_URL') ||
                    content.includes('createSupabaseClient(env)') ||
                    content.includes('new SupabaseClient(');
                // Allow in specific contexts
                const isAllowedContext = file.includes('/services/') || file.includes('/config/') || file.includes('.test.');
        ...
        it('should restrict service role key usage', () => {
            ...
            for (const file of allFiles) {
                ...
                if (hasServiceRole) {
                    ...
                    const isAllowedContext = file.includes('/services/') ||
    ...
    describe('Authentication Patterns', () => {
        it('should use enhanced auth middleware in protected routes', () => {
            ...
            for (const file of routeFiles) {
                ...
                const hasSensitiveOps = content.includes('.post(') || content.includes('.put(') || content.includes('.delete(');
                if (hasSensitiveOps) {
                    const hasAuthMiddleware = content.includes('enhancedAuth') ||
                        content.includes('secureAuth') ||
                        content.includes('requireAdmin');
                    // Allow for public endpoints and Discord webhook authentication
                    const isPublicEndpoint = content.includes('/health') ||
        ...
        it('should minimize dangerouslySetInnerHTML usage', () => {
            ...
            for (const file of componentFiles) {
                ...
                if (hasDangerousHTML) {
                    ...
                    const usesModularJS = content.includes('/client/auth.js') || content.includes('/client/forms.js');
                    const isSmallComponent = content.split('\n').length < 100;
                    // Allow Layout component with critical CSS
                    const isLayoutWithCriticalCSS = file.includes('Layout.tsx') && content.includes('Critical CSS');
    ...
    describe('Error Handling', () => {
        it('should not expose sensitive data in error messages', () => {
            const allFiles = getAllTSFiles('src').filter(file => !file.includes('.test.') && !file.includes('.old.'));
            for (const file of allFiles) {
                const content = readFile(file);
                if (!content)
                    continue;
                // Check for potential sensitive data in error messages
                const hasSensitiveError = /error.*password/i.test(content) ||
                ...
                if (hasSensitiveError) {
                    ...
                    const isInAuthService = file.includes('/services/auth') ||
