...
export class PlayersController {
    ...
    static async submitPlayer(c) {
        ...
        if (contentType?.includes('application/json')) {
            ...
            equipmentSetup = {};
            if (formData.get('blade_name'))
                equipmentSetup.blade_name = formData.get('blade_name');
            ...
            if (formData.get('year'))
                equipmentSetup.year = parseInt(formData.get('year'));
            if (formData.get('source_type'))
                equipmentSetup.source_type = formData.get('source_type');
            if (formData.get('source_url'))
                equipmentSetup.source_url = formData.get('source_url');
    ...
    static async addEquipmentSetup(c) {
        const slug = c.req.param('slug');
        const body = await c.req.json();
        if (!slug) {
