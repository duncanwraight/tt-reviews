...
export class AuthController {
     ...
     */
    static async signInSecure(c) {
        try {
            const { email, password } = await c.req.json();
            if (!email || !password) {
                throw new ValidationError('Email and authentication credentials are required');
            }
            const authService = c.get('authService') || new CookieAuthService(c.env);
            const { user, session, csrfToken, error } = await authService.signInWithCookie(c, email, password);
            if (error) {
                return errorResponse(c, error.message, 400);
            }
            return successResponse(c, {
     ...
     */
    static async getCSRFToken(c) {
        try {
            const authService = c.get('authService') || new CookieAuthService(c.env);
            const csrfToken = authService.setCSRFToken(c);
            return successResponse(c, { csrfToken });
        }
        catch (error) {
            console.error('Error generating CSRF token:', error);
            return errorResponse(c, 'Failed to generate CSRF token', 500);
        }
    }
}
