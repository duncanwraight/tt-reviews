...
export default function AdminPlayerSubmissions({ loaderData }: Route.ComponentProps) {
    ...
    <div className="space-y-6">
      ...
      {submissions.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">No player submissions to review at this time.</p>
        </div>
      ) : (
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {submission.playing_style && getPlayingStyleLabel(submission.playing_style)}
                          {submission.represents && ` • Represents: ${submission.represents}`}
                          {submission.birth_country && ` • Born: ${submission.birth_country}`}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    {submission.highest_rating && (
                      <div>
                        ...
                        <span className="ml-2 text-gray-900">{submission.highest_rating}</span>
                      </div>
                    )}
                    {submission.active_years && (
                      <div>
                        <span className="font-medium text-gray-700">Active Years:</span>
                        <span className="ml-2 text-gray-900">{submission.active_years}</span>
                      </div>
                    )}
                    {submission.playing_style && (
                      <div>
                        <span className="font-medium text-gray-700">Playing Style:</span>
                        <span className="ml-2 text-gray-900">{getPlayingStyleLabel(submission.playing_style)}</span>
                      </div>
                    )}
                  </div>

                  {submission.equipment_setup && Object.keys(submission.equipment_setup).length > 0 && (
                    <div className="mt-3 bg-gray-50 rounded-lg p-3">
                      <h4 className="text-sm font-medium text-gray-900 mb-2">Equipment Setup:</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                        {Object.entries(submission.equipment_setup).map(([key, value]) => (
                          <div key={key}>
