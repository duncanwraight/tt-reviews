...
interface EquipmentSubmissionRow extends Record<string, any> {
  id: string;
}
...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  const { data: submissions, error } =
    await loadPendingQueue<EquipmentSubmissionRow>(
      supabaseAdmin,
...
export default function AdminEquipmentSubmissions({
  ...
  // Disable Approve/Reject buttons across all listed rows while any
  // moderation submit is in flight — prevents a fast double-click and
  // any cross-row click during the action+loader round-trip.
