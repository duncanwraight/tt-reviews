...
export function PlayersHeader({ totalPlayers, user, countries, playingStyles, filters, pagination, players }: PlayersHeaderProps) {
  const buildFilterUrl = (newFilters: Partial<typeof filters>) => {
    ...
    if (combined.activeOnly !== undefined) params.set("active", combined.activeOnly.toString());
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-lg text-gray-600 max-w-3xl">
            Discover the equipment setups and playing styles of professional table
            tennis players from around the world. Learn what gear the pros use to
        ...
        <aside className="lg:w-64 flex-shrink-0">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="space-y-2">
              ...
              <Link
                to={buildFilterUrl({ sortBy: "highest_rating", sortOrder: "desc" })}
                className={`block p-3 rounded-lg transition-colors ${
                  filters.sortBy === "highest_rating" && filters.sortOrder === "desc"
                    ? "bg-purple-100 text-purple-800 font-medium"
                    : "text-gray-700 hover:bg-gray-100"
                }`}
              >
        ...
        <div className="flex-1">
          <div className="mb-6 flex items-center justify-between">
            <p className="text-gray-600">
              Showing {Math.min(pagination.limit, pagination.totalCount - (pagination.currentPage - 1) * pagination.limit)} of {pagination.totalCount} players
              {filters.gender && ` (${filters.gender === 'M' ? 'Men' : 'Women'})`}
              {filters.country && ` from ${COUNTRY_NAMES[filters.country] || filters.country}`}
              {filters.playingStyle && ` (${getPlayingStyleName(filters.playingStyle)})`}
            </p>
          </div>
