  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="border-t border-gray-100 pt-4">
        <h4 className="text-sm font-semibold text-gray-900 mb-2">Reviewer Context</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600">
          {review.reviewer_context.playing_level && (
            <div>
              <span className="font-medium">Level:</span> {review.reviewer_context.playing_level}
            </div>
          )}
          {review.reviewer_context.style_of_play && (
            <div>
              <span className="font-medium">Style:</span> {review.reviewer_context.style_of_play}
            </div>
          )}
          {review.reviewer_context.testing_duration && (
            <div>
              <span className="font-medium">Testing Duration:</span> {review.reviewer_context.testing_duration}
            </div>
          )}
          {review.reviewer_context.testing_type && (
            <div>
              <span className="font-medium">Testing Type:</span> {review.reviewer_context.testing_type}
            </div>
          )}
        </div>
      </div>
