...
3. **Write / update tests.** Every behavior change ships with tests. Run the full relevant suite (`npm run test`, `npm run test:e2e`, `npm run test:discord` as applicable) and make sure **all** tests pass — not just the new ones.
   ...
   - **For multi-child umbrellas, ship the relevant e2e alongside each child — never defer to a final cleanup ticket.** When children touch cross-cutting integration points (DB triggers with type-keyed `CASE`, Discord button dispatch ordering, `Record<SubmissionType, ...>` tables, modal field-name conventions), unit tests with mocked clients pass through silent gaps that only the full submit → moderate → verify lifecycle catches. Scoping e2e into the last child is how TT-105 shipped two production 500/400s before anyone tested end-to-end. See `docs/E2E.md`.
...
5. **Wait for CI to pass** before moving on. Run `./scripts/ci-wait.sh` (with `run_in_background: true` so the conversation isn't blocked — the harness notifies on completion). The wrapper picks the latest run on the current branch by default, polls until the run completes, and exits non-zero on a non-success conclusion with the failed steps already dumped. See `CLAUDE.md` "Checking CI status" for why this is preferred over `gh run watch`. If CI fails, fix on the same item — don't open the next one on top of a red build. Only after CI is green (and, where applicable, the PR is merged / deployed) move the item to Completed with `./scripts/plane.sh done TT-N`.
